    ...
    function escolasCriticasPorRegiao(){
        ...
        WITH escolas_base AS (
            SELECT 
                id_escola,
                regiao,
                inse_classificacao2014,
                inse_classificacao2015,
                CASE 
                    WHEN valor_inse < 5 THEN 1
                    ELSE 0
                END AS critica
            FROM Escola
            GROUP BY id_escola, regiao, inse_classificacao2014, inse_classificacao2015, valor_inse
        ),
        todas_regioes AS (
        ...
        estatistica AS (
            SELECT
                regiao,
                MAX(inse_classificacao2014) AS inse_2014,
                MAX(inse_classificacao2015) AS inse_2015,
        ...
        SELECT
            r.regiao,
            COALESCE(e.inse_2014, '—') AS inse_2014,
            COALESCE(e.inse_2015, '—') AS inse_2015,
